# -*- coding: utf-8 -*-
"""
Evaluate disruptor rules and scenario sensitivity from
6_JSONL_component_normalized.jsonl

Design
------
This script applies a transparent rule-based disruptor mapping to garment-level
JSONL records. It follows the same readable layout as the category-normalization
and sorting-rule scripts:

1. regex inventories are declared explicitly at the top
2. include and exclude patterns are separated
3. conservative / default / expanded scenario settings are visible in code
4. main evaluation proceeds rule by rule in a fixed order
5. row-level outputs and scenario summaries are exported

Repository-oriented output filenames
------------------------------------
Output files use descriptive repository filenames rather than the earlier
number-prefixed working filenames.

Update note
-----------
Compared with the earlier disruptor script, R7 and R8 now borrow the improved
surface-reference / hidden-layer logic from the newer sorting-rule evaluation:
- choose one surface reference in this order:
    1) coating as outermost readable surface
    2) first surface_component in original order
    3) first component overall
- identify hidden structural components using component_class / component_name
- compare hidden/secondary materials against the chosen surface reference

This lets the script distinguish:
- presence of lining / multilayer structure
- presence of secondary components

The disruptor framework distinguishes:
- removable disruptors (hard-hardware cues)
- retained / non-removable disruptors or barriers

R1-R3 note
----------
R1-R3 are cue-based hard-hardware indicators, not verified material-composition
classes. Retailer product pages usually disclose hardware function (e.g. zip,
buckle, snap) rather than the material of that hardware. The rule boundaries are
therefore defined as follows:
- R1: metal-specified hardware cues, including explicit metal wording and a
  small set of strongly metal-conventional garment hardware terms
- R2: plastic-specified hardware cues, including explicit plastic / polymer /
  resin wording
- R3: material-ambiguous hardware cues, including generic hard-hardware terms
  whose material cannot be inferred reliably from retailer text

R3 is retained as a core removable-disruptor indicator, but it is no longer
derived from R1 + R2 co-occurrence. If R1 and R2 are both triggered, the garment
is already captured through the aggregate removable-disruptor indicator; it is
not additionally assigned to R3 unless an explicit R3 ambiguous cue is present.
"""
...
output_dir = Path("disruptor_eval_outputs")
...
# ---------------------------------------------------------
# R1 Metal-specified or metal-conventional hard-hardware cues
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# R2 Plastic-specified hard-hardware cues
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# R3 Material-ambiguous hard-hardware cues
# ---------------------------------------------------------
...
# as a non-mono hard-hardware cue, but R3 is no longer derived from R1 + R2
# co-occurrence.
        ...
        for setting in ["conservative", "default", "expanded"]:
            ...
            if r8_hit:
                if setting == "conservative":
                    ...
                    # any detected secondary component with material info counts as expanded structural difference signal
                    valid_secondary = [
                        comp for comp in secondary_components
                        if norm_text(comp.get("component_name_norm"))
                        and norm_text(comp.get("component_name_norm")) != surface_ref_name
                        and norm_text(comp.get("component_name_norm")) != "pocket"
                        and norm_text(comp.get("component_name_norm")) in R8B_ALLOWED_COMPONENT_NAMES
                    ]
                    if valid_secondary:
                        first_sec = norm_text(valid_secondary[0].get("component_name_norm"))
                        r8b_hit = True
                        r8b_sig = f"{surface_ref_name}__{first_sec}"
            ...
            # Aggregate retained-barrier indicators follow the core rule framework
...
# =========================================================
# This diagnostic asks: among garment variants with detected hard-hardware cues,
...
with open(summary_txt_out, "w", encoding="utf-8") as fsum:
    fsum.write("Disruptor rule evaluation summary (updated with surface/hidden logic and cue-based R1-R3 hardware boundaries)\n\n")
    fsum.write(f"Input file: {input_file}\n")
    fsum.write(f"Processed rows: {n_total}\n\n")
    fsum.write("R1-R3 hardware note\n")
    fsum.write("=" * 60 + "\n")
    fsum.write("R1-R3 are cue-based hard-hardware indicators, not verified material-composition classes.\n")
    fsum.write("R1 = metal-specified / strongly metal-conventional cues; R2 = plastic-specified cues; R3 = material-ambiguous cues. R3 is not derived from R1 + R2 co-occurrence.\n\n")
